...
impl App {
    ...
    pub fn quit(&self) {
        self.draw_state.renderers.borrow_mut().clear();
    ...
    E: LoopEvent,
{
    ...
    event_loop.run(move |mut event, event_loop_window_target, control_flow| {
        ...
        if let Some(model) = model.as_mut() {
            let _ = process_and_emit_winit_event::<M, E>(&mut app, model, event_fn, &event);
        }
